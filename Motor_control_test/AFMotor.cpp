...
DigitalOut motorLatch (MOTORLATCH);
DigitalOut motorEnable (MOTORENABLE);
DigitalOut motorData (MOTORDATA);
DigitalOut motorClock (MOTORCLK);
...
static AFMotorController MC; // 다양한 인스턴스 간에 데이터 공유와 접근을 가능

/******************************************
               MOTORS
******************************************/
inline void initPWM1(uint32_t freq) {
    ...
    TCCR1A |= (1 << COM1A1) | (1 << WGM10); // fast PWM, turn on oc1a
  ...
  //  #error "This chip is not supported!"
        ...
        //pinMode(11, OUTPUT);
        DigitalOut motorPin (D11);
    #endif
}

inline void setPWM1(uint32_t s) {
#if defined(__AVR_ATmega8__) || \
    defined(__AVR_ATmega48__) || \
    defined(__AVR_ATmega88__) || \
    defined(__AVR_ATmega168__) || \
...
inline void initPWM2(uint32_t freq) {
#if defined(__AVR_ATmega8__) || \
    defined(__AVR_ATmega48__) || \
    defined(__AVR_ATmega88__) || \
    defined(__AVR_ATmega168__) || \
  ...
  //  #error "This chip is not supported!"
    ...
    //pinMode(3, OUTPUT);
    DigitalOut motorPin (D3);
}

inline void setPWM2(uint32_t s) {
#if defined(__AVR_ATmega8__) || \
    defined(__AVR_ATmega48__) || \
    defined(__AVR_ATmega88__) || \
    defined(__AVR_ATmega168__) || \
...
inline void initPWM3(uint32_t freq) {
#if defined(__AVR_ATmega8__) || \
    defined(__AVR_ATmega48__) || \
    defined(__AVR_ATmega88__) || \
    defined(__AVR_ATmega168__) || \
    ...
    TCCR4A |= (1 << COM1A1) | (1 << WGM10); // fast PWM, turn on oc4a
    TCCR4B = (freq & 0x7) | (1 << WGM12);
    //TCCR4B = 1 | _BV(WGM12);
    OCR4A = 0;
#elif defined(__PIC32MX__)
    if (!MC.TimerInitalized)
...
#endif
    //pinMode(6, OUTPUT);
    DigitalOut motorPin (D6);
}

inline void setPWM3(uint32_t s) {
#if defined(__AVR_ATmega8__) || \
    defined(__AVR_ATmega48__) || \
    defined(__AVR_ATmega88__) || \
    defined(__AVR_ATmega168__) || \
...
inline void initPWM4(uint32_t freq) {
#if defined(__AVR_ATmega8__) || \
    defined(__AVR_ATmega48__) || \
    defined(__AVR_ATmega88__) || \
    defined(__AVR_ATmega168__) || \
    ...
    TCCR3A |= (1 << COM1A1) | (1 << WGM10); // fast PWM, turn on oc3a
    TCCR3B = (freq & 0x7) | (1 << WGM12);
    //TCCR4B = 1 | _BV(WGM12);
    OCR3A = 0;
#elif defined(__PIC32MX__)
...
#endif
    //pinMode(5, OUTPUT);
    DigitalOut motorPin (D5);
}

inline void setPWM4(uint32_t s) {
#if defined(__AVR_ATmega8__) || \
    defined(__AVR_ATmega48__) || \
    defined(__AVR_ATmega88__) || \
    defined(__AVR_ATmega168__) || \
...
// 모터 핀 선택
AF_DCMotor::AF_DCMotor(uint32_t num, uint32_t freq) {
  motornum = num;
  pwmfreq = freq;
  ...
  switch (num) {
  case 1:
    latch_state &= ~(1 << MOTOR1_A) & ~(1 << MOTOR1_B); // 두 핀의 모터 0으로 설정
    MC.latch_tx();
    initPWM1(freq);
    break;
  case 2:
    latch_state &= ~(1 << MOTOR2_A) & ~(1 << MOTOR2_B); // set both motor pins to 0
    MC.latch_tx();
    initPWM2(freq);
    break;
  case 3:
    latch_state &= ~(1 << MOTOR3_A) & ~(1 << MOTOR3_B); // set both motor pins to 0
    MC.latch_tx();
    initPWM3(freq);
    break;
  case 4:
    latch_state &= ~(1 << MOTOR4_A) & ~(1 << MOTOR4_B); // set both motor pins to 0
    MC.latch_tx();
    initPWM4(freq);
    break;
  }
...
// 모터 동작 시스템
void AF_DCMotor::run(uint32_t cmd) {
  uint8_t a, b;
  switch (motornum) {
  case 1:
  ...
  case BACKWARD:
    ...
    break;
  case RELEASE:
    latch_state &= ~(1 << a);     // A and B both low
    latch_state &= ~(1 << b); 
    MC.latch_tx();
    break;
  }
...
void AF_DCMotor::setSpeed(uint32_t speed) {
  switch (motornum) {
  case 1:
    setPWM1(speed); break;
  case 2:
